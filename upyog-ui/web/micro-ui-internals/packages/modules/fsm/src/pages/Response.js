...
const Response = (props) => {
  ...
  const handleResponse = () => {
    if (Data?.fsm?.[0].paymentPreference === "POST_PAY") {
      ...
      history.push(`/suda-ui/employee/payment/collect/FSM.TRIP_CHARGES/${state?.applicationData?.applicationNo || Data?.fsm?.[0].applicationNo}`);
    }
  };
  ...
  useEffect(() => {
    switch (selectedAction) {
      case "GO_TO_HOME":
        return isCitizen ? history.push("/suda-ui/citizen") : history.push("/suda-ui/employee");
      case "ASSIGN_TO_DSO":
        return history.push(`/suda-ui/employee/fsm/application-details/${getApplicationNo}`);
      case "PAY":
