    ...
    return useMemo( () => ([
        {
          ...
          Cell: ({ row }) => {
            return (
              <div>
                <span className="link">
                  <Link to={window.location.href.includes("/citizen")  ? `/suda-ui/citizen/obps/${getRedirectionLink(row.original["businessService"]) || "--"}/${row.original["applicationNo"] || row.original["applicationNumber"]}` : `/suda-ui/employee/obps/${getRedirectionLink(row.original["businessService"]) || "--"}/${row.original["applicationNo"] || row.original["applicationNumber"]}`}>
