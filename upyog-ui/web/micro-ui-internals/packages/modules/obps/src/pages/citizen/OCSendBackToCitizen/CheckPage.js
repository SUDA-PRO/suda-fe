...
const CheckPage = ({ onSubmit, value }) => {
  ...
  let routeLink = `/suda-ui/citizen/obps/sendbacktocitizen/${value.businessService=="BPA_OC" ? "ocbpa" : "bpa"}/${value?.tenantId}/${value?.applicationNo}`;
  if (value?.uiFlow?.flow === "OCBPA") routeLink = `/suda-ui/citizen/obps/sendbacktocitizen/ocbpa/${value?.tenantId}/${value?.applicationNo}`;
    ...
    mutation.mutate(
      { BPA: { ...formdata?.BPA, workflow } },
      {
        ...
        onSuccess: (data, variables) => {
          ...
          history.replace(`/suda-ui/citizen/obps/sendbacktocitizen/ocbpa/${value?.tenantId}/${value?.applicationNo}/acknowledgement`, { data: value?.applicationNo });
        },
      }
    );
  }
