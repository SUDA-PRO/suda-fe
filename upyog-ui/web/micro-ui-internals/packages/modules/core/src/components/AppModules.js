...
export const AppModules = ({ stateCode, userType, modules, appTenants }) => {
  ...
  const appRoutes = modules.map(({ code, tenants }, index) => {
    ...
    <Redirect to={{ pathname: "/suda-ui/employee/user/error?type=notfound", state: { from: location.pathname + location.search } }} />
  </Route>;
  });

  return (
    <div className="ground-container" style={{ paddingBottom: 0, marginBottom: 0 }}>
      <Switch>
        {appRoutes}
        <Route path={`${path}/login`}>
          <Redirect to={{ pathname: "/suda-ui/login", state: { from: location.pathname + location.search } }} />
        </Route>
