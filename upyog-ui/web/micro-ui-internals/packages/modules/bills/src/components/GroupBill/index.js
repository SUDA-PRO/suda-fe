...
const GroupBills = ({ tenantId, onSubmit, data, count, isLoading, resultOk,serviceType,locality }) => {
    ...
    const handleBillLinkClick = (row) => {
        history.push(`/suda-ui/employee/bills/bill-details?connectionNumber=${row?.consumerCode}&tenantId=${tenantId}&service=${row?.businessService}&from=ABG_CANCEL_BILL`, row)
    }
    ...
    const handleCancelBillAction = (_data) => {
        ...
        history.push("/suda-ui/employee/bills/response-cancelBill", { filters, currentBill });
