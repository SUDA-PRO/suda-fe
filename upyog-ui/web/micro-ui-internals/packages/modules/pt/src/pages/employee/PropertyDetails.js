  ...
  useEffect(() => {
    if (appDetailsToShow?.applicationDetails?.[0]?.values?.[1].title !== "PT_TOTAL_DUES") {
      appDetailsToShow?.applicationDetails?.unshift({
        ...
        belowComponent: () => (
          <LinkLabel
            onClick={() => history.push({ pathname: `/suda-ui/employee/pt/payment-details/${applicationNumber}`})}
            ...
            ? [
              {
                ...
                customFunctionToExecute: (data) => {
                  ...
                  history.replace({ pathname: `/suda-ui/employee/pt/ptsearch/assessment-details/${applicationNumber}`, state: { ...data } });
                },
              ...
              },
              {
                ...
                redirectionUrl: {
                  pathname: !fetchBillData?.Bill[0]?.totalAmount
                    ? `/suda-ui/employee/pt/property-mutate-docs-required/${applicationNumber}`
                    : `/suda-ui/employee/payment/collect/PT/${applicationNumber}`,
              ...
              },
              {
                ...
                customFunctionToExecute: (data) => {
                history.push("/suda-ui/employee/pt/response", { Property: data.Property, key: "UPDATE", action: "SUBMIT" });
                },
                // redirectionUrl: {
  ...
  if (appDetailsToShow?.applicationData?.status === "ACTIVE" && PT_CEMP) {
    ...
    if (!workflowDetails?.data?.actionState?.nextActions?.find((e) => e.action === "UPDATE")) {
      workflowDetails?.data?.actionState?.nextActions?.push({
        action: "UPDATE",
        redirectionUrl: {
          pathname: `/suda-ui/employee/pt/modify-application/${applicationNumber}`,
