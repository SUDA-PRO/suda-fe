...
setCityCode(city.code);
  ...
  const handleNewSubmit = () => {
    // build data object matching onPropertySearch expectations
    const data = {
      city: uiCity,
      locality: uiLocality,
      mobileNumber: uiMobile,
      propertyIds: uiPropertyId,
      oldPropertyId: uiOldPropertyId,
      doorNumber: uiDoorNo,
      name: uiOwnerName,
    };
          ...
          {currentAction === 0 ? (
            <div style={{ display: "grid", gridTemplateColumns: isMobile ? "1fr" : "1fr 1fr 1fr", gap: "18px" }}>
              <div>
                ...
                <input type="tel" value={uiMobile} onChange={(e) => setUiMobile(e.target.value)} placeholder="e.g. 9876543210" style={inputStyle} onFocus={onInputFocus} onBlur={onInputBlur} />
              </div>
              <div>
                <FieldLabel text={t(property.label) || "Property ID"} />
                <input type="text" value={uiPropertyId} onChange={(e) => setUiPropertyId(e.target.value)} placeholder="e.g. UP-PT-2024-01-01-000001" style={inputStyle} onFocus={onInputFocus} onBlur={onInputBlur} />
              </div>
              <div>
                <FieldLabel text={t(oldProperty.label) || "Old Property ID"} />
                <input type="text" value={uiOldPropertyId} onChange={(e) => setUiOldPropertyId(e.target.value)} style={inputStyle} onFocus={onInputFocus} onBlur={onInputBlur} />
              </div>
            </div>
          ) : (
            <div style={{ display: "grid", gridTemplateColumns: isMobile ? "1fr" : "1fr 1fr", gap: "18px" }}>
              <div>
                <FieldLabel text={t(doorNumber.label) || "Door / House Number"} />
                <input type="text" value={uiDoorNo} onChange={(e) => setUiDoorNo(e.target.value)} style={inputStyle} onFocus={onInputFocus} onBlur={onInputBlur} />
              </div>
              <div>
                <FieldLabel text={t(name.label) || "Owner Name"} />
                <input type="text" value={uiOwnerName} onChange={(e) => setUiOwnerName(e.target.value)} style={inputStyle} onFocus={onInputFocus} onBlur={onInputBlur} />
              </div>
            </div>
          )}
        </div>
        ...
        {isOBPS && onSkip && (
          <div style={{ textAlign: "center" }}>
            <button type="button" onClick={onSkip} style={{ background: "none", border: "none", color: "#f47738", fontSize: "13px", fontWeight: "700", cursor: "pointer", textDecoration: "underline" }}>
              {t("CORE_COMMON_SKIP_CONTINUE") || "Skip & Continue"}
            </button>
          </div>
        )}
        {isOBPS && !onSkip && (
          <div style={{ textAlign: "center" }}>
            <Link to="/suda-ui/citizen/obps/bpa/building_plan_scrutiny/new_construction/location" style={{ color: "#f47738", fontSize: "13px", fontWeight: "700" }}>{t("CORE_COMMON_SKIP_CONTINUE")}</Link>
          </div>
        )}
      </div>
