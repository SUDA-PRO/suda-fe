...
const PropertySearchNSummary = ({ config, onSelect, userType, formData, setError, formState, clearErrors }) => {
  ...
  const searchProperty = () => {
    ...
    if(window.location.pathname.includes("/tl/new-application")){
      history.push(`/suda-ui/employee/tl/new-application?propertyId=${propertyId}`)
      ...
      history.push(`/suda-ui/employee/ws/new-application?propertyId=${propertyId}`)
  };
  ...
  return (
    <React.Fragment>
     {(window.location.href.includes("/tl/") ? (!(formData?.tradedetils?.[0]?.structureType?.code === "MOVABLE") && (isEmpNewApplication || isEmpRenewLicense) ) : true) && <div className="fsm-property-search">
      <LabelFieldPair>
      ...
      <span onClick={() => history.push(`/suda-ui/employee/commonpt/search?redirectToUrl=${redirectBackUrl}&${serachParams}`, { ...state })}>
        <LinkButton label={t("CPT_SEARCH_PROPERTY")} style={{ color: "#a82227", display: "inline-block" }} />
      </span>
      &nbsp; | &nbsp;
      <span onClick={() => history.push(`/suda-ui/employee/commonpt/new-application?redirectToUrl=${redirectBackUrl}&${serachParams}`, { ...state })}>
