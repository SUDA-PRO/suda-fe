...
const CHBAcknowledgement = ({ data, onSuccess }) => {
  ...
  const handleMakePayment = async () => {
    ...
    if (isSlotBooked) {
      ...
      history.push({
        pathname: `/suda-ui/citizen/payment/my-bills/${"chb-services"}/${mutation.data?.hallsBookingApplication[0].bookingNo}`,
        ...
        history.push({
          pathname: `/suda-ui/employee/payment/collect/${"chb-services"}/${mutation.data?.hallsBookingApplication[0].bookingNo}`,
          state: { tenantId:tenantId, bookingNo: mutation.data?.hallsBookingApplication[0].bookingNo,timerValue:result?.data?.timerValue,SlotSearchData:SlotSearchData },
        });
      }
  }catch (error) {
  ...
  return mutation.isLoading || mutation.isIdle ? (
    ...
    <Card>
      ...
      <div style={{ display: 'flex', flexDirection: 'row', gap: '20px' }}>
        {user.type==="EMPLOYEE" &&(<Link to={`/suda-ui/employee`}>
        <SubmitBar label={t("CORE_COMMON_GO_TO_HOME")} />
         </Link>)}
         {user.type==="CITIZEN" &&(<Link to={`/suda-ui/citizen`}>
        <SubmitBar label={t("CORE_COMMON_GO_TO_HOME")} />
         </Link>)}
