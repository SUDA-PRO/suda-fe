...
const NewApplication = () => {
  ...
  const onSubmit = (data) => {
          ...
          Digit.TLService.update({ Licenses: [licenses] }, tenantId)
            .then((response) => {
              if (response?.Licenses?.length > 0) {
                ...
                history.replace(`/suda-ui/employee/tl/response`, { data: response?.Licenses });
    ...
    // history.replace("/suda-ui/employee/tl/response", { Licenses: [formData], documents: applicationDocuments });
    // history.push("/suda-ui/employee/pt/response", { Property: formData });
    // history.push("/suda-ui/employee/pt/response", { Property: _formData });
  };
