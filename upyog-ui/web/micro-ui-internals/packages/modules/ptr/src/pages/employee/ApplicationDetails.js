...
const ApplicationDetails = () => {
    ...
    workflowDetails.data.actionState.nextActions = workflowDetails?.data?.actionState?.nextActions.map((act) => {
      if (act.action === "PAY") {
        return {
          ...
          redirectionUrl: { pathname: `/suda-ui/employee/payment/collect/pet-services/${appDetailsToShow?.applicationData?.applicationData?.applicationNumber}` },
        };
      }
      return act;
    });
  ...
  dowloadOptions.push({
    ...
    <div>
      ...
      {(appDetailsToShow?.applicationData?.applicationData?.status == "Expired") && <Link to={`/suda-ui/employee/ptr/petservice/revised-application`}  >
