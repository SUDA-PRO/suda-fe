    ...
    return [
      {
        ...
        Cell: ({ row }) => {
          return (
            <span className="link">
              <Link to={`/suda-ui/employee/receipts/details/${row.original?.paymentDetails[0]?.businessService}/${encodeURIComponent(row.original?.paymentDetails[0]?.receiptNumber)}`}>{row.original?.paymentDetails[0]?.receiptNumber}</Link>
            </span>
          );
        },
      },
            ...
            allLinks={[
              {
                ...
                link: "/suda-ui/employee/receipts/inprogress",
