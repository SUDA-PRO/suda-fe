...
const ApplicationBillAmendment = () => {
  ...
  useEffect(() => {
    const timer = setTimeout(() => {
      if (goToAppDetailsPage) window.location.href = `${window.location.origin}/suda-ui/employee/ws/generate-note-bill-amendment?applicationNumber=${state?.data.applicationDetails.amendment.amendmentId}`
    }, 5000);
  ...
  const onFormSubmit = (d) => {
      ...
      mutate({ AmendmentUpdate }, {
        ...
        onSuccess: (data, variables) => {
          ...
          if (data?.Amendments?.length > 0) {
            ...
            // history?.push(`/suda-ui/employee/ws/application-details-bill-amendment?applicationNumber=${data?.Amendments[0].amendmentId}`)
            return
          }
        },
      })
    ...
    history.push("/suda-ui/employee/ws/response", data);
  };
