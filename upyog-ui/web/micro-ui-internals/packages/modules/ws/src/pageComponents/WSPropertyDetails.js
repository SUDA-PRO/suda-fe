...
const WSPropertyDetails = ({ t, config, onSelect, userType, formData, setError, formState, clearErrors }) => {
  const redirectBackUrl = `/suda-ui/${userType}/ws/new-application`;
    ...
    <React.Fragment>
      ...
      {propertyDetails && propertyDetails?.Properties.length && (
        <React.Fragment>
          ...
          <Link to={`/suda-ui/employee/commonpt/view-property?propertyId=${propertyId}&tenantId=${tenantId}`}>
