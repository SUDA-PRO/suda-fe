...
const WSWFApplicationTimeline = (props) => {
  ...
  const getTimelineCaptions = (checkpoint) => {
    if (checkpoint.state === "OPEN") {
      ...
      return (
        <div>
          <Link to={`/suda-ui/citizen/commonpt/view-property?propertyId=${props?.application?.propertyId}&tenantId=${props?.application?.tenantId}`}>
    ...
    //   return (
    //     <div>
    //       <Link to={`/suda-ui/citizen/pt/property/properties/${props?.application?.propertyId}`}>
    ...
    switch (nextAction?.action) {
      ...
      { if(props?.paymentbuttonenabled !== false)  return (
          <div style={{ marginTop: "1em", bottom: "0px", width: "100%", marginBottom: "1.2em" }}>
            <Link
              to={{ pathname: `/suda-ui/citizen/payment/collect/${businessService}/${props.id}?consumerCode=${props.id}&&workflow=WNS`, state: { tenantId: props.application.tenantId } }}
            >
        ...
        return (
          <div style={{ marginTop: "1em", bottom: "0px", width: "100%", marginBottom: "1.2em" }}>
            {businessService != "PT.MUTATION" && (
              <Link
                to={{
                  pathname: `/suda-ui/citizen/pt/property/edit-application/action=edit-${businessService}/${props.id}`,
