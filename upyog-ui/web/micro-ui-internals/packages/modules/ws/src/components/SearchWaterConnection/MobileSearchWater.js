        ...
        return (
          <div>
            <span className="link">
              <Link
                to={{
                  pathname: `/suda-ui/employee/payment/collect/${data?.["service"] == "WATER" ? "WS" : "SW"}/${encodeURIComponent(
                    data?.["connectionNo"]
  ...
  const GetStatusLinkCell = (value) => {
    ...
    return (
      <div>
        <span className="link">
          <Link
            to={`/suda-ui/employee/ws/connection-details?applicationNumber=${value?.connectionNo}&tenantId=${
              value?.tenantId
