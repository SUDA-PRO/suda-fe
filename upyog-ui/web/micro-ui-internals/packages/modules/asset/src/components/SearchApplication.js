...
const ASSETSearchApplication = ({ isLoading, t, onSubmit, data, count, setShowToast, ActionBarStyle = {}, MenuStyle = {}, parentRoute, tenantId }) => {
  ...
  const processDepreciation = async (applicationNo, assetId) => {
    try {
      ...
      history.replace("/suda-ui/employee/asset/assetservice/asset-process-depreciation-response", { ProcessDepreciation: applicationDetails,  applicationNo});
    } catch (error) {
  ...
  const columns = useMemo(() => ([
    {
      ...
      Cell: ({ row }) => {
        return (
          <div>
            <span className="link">
              <Link to={`/suda-ui/employee/asset/assetservice/applicationsearch/application-details/${row.original?.["applicationNo"]}`}>
 ...
 //later will convert it into the action bar same as i have iused in ApplicationDetailsActionBar.js file in template
    {
      ...
      Cell: ({ row }) => {
        ...
        //       ? `/suda-ui/employee/asset/assetservice/return-assets/${row?.original?.["applicationNo"]}`
        //       : `/suda-ui/employee/asset/assetservice/assign-assets/${row?.original?.["applicationNo"]}`,
        //   },
        //   {
        //     label: t("AST_DISPOSE"),
        //     link: `/suda-ui/employee/asset/assetservice/dispose-assets/${row?.original?.["applicationNo"]}`,
        //   },
        //   {
        //     label: t("AST_DEPRECIATION"),
        //     link: `/suda-ui/employee/asset/assetservice/depreciate-assets/${row?.original?.["applicationNo"]}`,
        //   },
        // ];
  ...
  const downloadQRReport = async () => {
    ...
    for (let i = 0; i < data.length; i += batchSize) {
      ...
      const qrPromises = batch.map(async (row, index) => {
        // const url = `https://niuatt.niua.in/suda-ui/employee/asset/assetservice/applicationsearch/application-details/${row.applicationNo}`;
        const url = `${base_url}/suda-ui/citizen/assets/services?tenantId=${tenantId}&applicationNo=${row.applicationNo}`;
