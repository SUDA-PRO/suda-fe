...
const BPASearchProperty = ({ config, onSelect, onSkip }) => {
  ...
  useEffect(() => {
    if (propertyData && !propertyDataLoading && !error) {
      if (propertyData?.Properties?.length > 0) {
        // Pass results forward; CPTSearchResults will display them
        onSelect(config?.key, { ...searchData, Properties: propertyData.Properties });
      } else {
  ...
  }, [propertyData, propertyDataLoading, error]);
  ...
  const handleSubmit = () => {
    ...
    setSearchData({ city: formCity.code, filters });
  };
